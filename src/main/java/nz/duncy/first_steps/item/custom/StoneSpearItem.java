...
public class StoneSpearItem extends SpearItem {
    public StoneSpearItem(ToolMaterial toolMaterial, float attackDamage, Item.Settings settings) {
    ...
    public SpearEntity spawnWithVelocity(ServerWorld serverWorld, ItemStack stack, PlayerEntity playerEntity) {
        return ProjectileEntity.spawnWithVelocity(StoneSpearEntity::new, serverWorld, stack, playerEntity, 0.0F, 2.5F, 1.0F);
    }

    @Override
    public SpearEntity getEntity(World world, Position pos, ItemStack stack, Direction direction) {
        return new StoneSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
    }
}
