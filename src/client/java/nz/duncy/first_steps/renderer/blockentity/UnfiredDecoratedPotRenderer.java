...
import nz.duncy.first_steps.renderer.blockentity.state.UnfiredDecoratedPotRenderState;
import nz.duncy.first_steps.world.level.block.entity.UnfiredDecoratedPotBlockEntity;

import org.joml.Vector3fc;
import org.jspecify.annotations.Nullable;

public class UnfiredDecoratedPotRenderer implements BlockEntityRenderer<UnfiredDecoratedPotBlockEntity, UnfiredDecoratedPotRenderState> {
    ...
    public UnfiredDecoratedPotRenderer(BlockEntityRendererProvider.Context context) {
        this(context.entityModelSet(), context.materials());
    }

    public UnfiredDecoratedPotRenderer(SpecialModelRenderer.BakingContext bakingContext) {
        this(bakingContext.entityModelSet(), bakingContext.materials());
    }

    public UnfiredDecoratedPotRenderer(EntityModelSet entityModelSet, MaterialSet materialSet) {
        this.materials = materialSet;
        ModelPart modelPart = entityModelSet.bakeLayer(ModModelLayers.UNFIRED_DECORATED_POT_BASE);
        this.neck = modelPart.getChild(NECK);
        this.top = modelPart.getChild(TOP);
        this.bottom = modelPart.getChild(BOTTOM);
        ModelPart modelPart2 = entityModelSet.bakeLayer(ModModelLayers.UNFIRED_DECORATED_POT_SIDES);
   ...
   public static LayerDefinition createSidesLayer() {
    ...
    public UnfiredDecoratedPotRenderState createRenderState() {
        return new UnfiredDecoratedPotRenderState();
    }

    public void extractRenderState(UnfiredDecoratedPotBlockEntity UnfiredDecoratedPotBlockEntity, UnfiredDecoratedPotRenderState UnfiredDecoratedPotRenderState, float f, Vec3 vec3, ModelFeatureRenderer.@Nullable CrumblingOverlay crumblingOverlay) {
        BlockEntityRenderer.super.extractRenderState(UnfiredDecoratedPotBlockEntity, UnfiredDecoratedPotRenderState, f, vec3, crumblingOverlay);
        UnfiredDecoratedPotRenderState.decorations = UnfiredDecoratedPotBlockEntity.getDecorations();
        UnfiredDecoratedPotRenderState.direction = UnfiredDecoratedPotBlockEntity.getDirection();
    }

    public void submit(UnfiredDecoratedPotRenderState UnfiredDecoratedPotRenderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        poseStack.pushPose();
        Direction direction = UnfiredDecoratedPotRenderState.direction;
        poseStack.translate(0.5, 0.0, 0.5);
        poseStack.mulPose(Axis.YP.rotationDegrees(180.0F - direction.toYRot()));
        poseStack.translate(-0.5, 0.0, -0.5);

        this.submit(poseStack, submitNodeCollector, UnfiredDecoratedPotRenderState.lightCoords, OverlayTexture.NO_OVERLAY, UnfiredDecoratedPotRenderState.decorations, 0);
        poseStack.popPose();
    }
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int i, int j, PotDecorations potDecorations, int k) {
        RenderType renderType = ModSheets.UNFIRED_DECORATED_POT_BASE.renderType(RenderTypes::entitySolid);
        TextureAtlasSprite textureAtlasSprite = this.materials.get(ModSheets.UNFIRED_DECORATED_POT_BASE);
