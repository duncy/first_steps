...
import nz.duncy.first_steps.renderer.blockentity.state.DecoratedJarRenderState;
import nz.duncy.first_steps.world.level.block.entity.DecoratedJarBlockEntity;

import org.joml.Vector3fc;
import org.jspecify.annotations.Nullable;

public class DecoratedJarRenderer implements BlockEntityRenderer<DecoratedJarBlockEntity, DecoratedJarRenderState> {
    ...
    public DecoratedJarRenderer(BlockEntityRendererProvider.Context context) {
        this(context.entityModelSet(), context.materials());
    }

    public DecoratedJarRenderer(SpecialModelRenderer.BakingContext bakingContext) {
        this(bakingContext.entityModelSet(), bakingContext.materials());
    }

    public DecoratedJarRenderer(EntityModelSet entityModelSet, MaterialSet materialSet) {
        this.materials = materialSet;
        ModelPart modelPart = entityModelSet.bakeLayer(ModModelLayers.DECORATED_JAR_BASE);
        this.neck = modelPart.getChild(NECK);
        this.top = modelPart.getChild(TOP);
        this.bottom = modelPart.getChild(BOTTOM);
        ModelPart modelPart2 = entityModelSet.bakeLayer(ModModelLayers.DECORATED_JAR_SIDES);
    ...
    public DecoratedJarRenderState createRenderState() {
        return new DecoratedJarRenderState();
    }

    public void extractRenderState(DecoratedJarBlockEntity decoratedJarBlockEntity, DecoratedJarRenderState decoratedJarRenderState, float f, Vec3 vec3, ModelFeatureRenderer.@Nullable CrumblingOverlay crumblingOverlay) {
    ...
    public void submit(DecoratedJarRenderState decoratedJarRenderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        poseStack.pushPose();
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int i, int j, PotDecorations potDecorations, int k) {
        RenderType renderType = ModSheets.DECORATED_JAR_BASE.renderType(RenderTypes::entitySolid);
        TextureAtlasSprite textureAtlasSprite = this.materials.get(ModSheets.DECORATED_JAR_BASE);
